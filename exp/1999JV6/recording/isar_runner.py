...
class ISARRunner:
    def __init__(self, conf_path, mode='train', case='CASE_NAME', is_continue=False, checkpoint_name=None):
        ...
        self.intensity_freeze_end = self.conf.get_int('train.intensity_freeze_end', default=0)
    ...
    def render_isar(self, frame_meta, image_shape, cos_anneal_ratio=1.0):
        ...
        return self.renderer.render_frame_nerf(
            ...
            self.nerf_network,
            image_shape=image_shape,
            use_learned_intensity=self.iter_step >= self.intensity_freeze_end
        )
    ...
    def load_checkpoint(self, checkpoint_name):
        ...
        if self.model_type == 'neus':
            ...
            checkpoint_density_mode = checkpoint.get('nerf_density_mode')
            current_density_mode = self.nerf_network.density_mode
            if checkpoint_density_mode != current_density_mode:
                raise ValueError(
                    f'Checkpoint NeRF density mode {checkpoint_density_mode!r} does not match '
                    f'current mode {current_density_mode!r}; start a fresh run after changing geometry mode.'
                )
    ...
    def save_checkpoint(self):
        ...
        if self.model_type == 'neus':
            ...
            checkpoint['nerf_density_mode'] = self.nerf_network.density_mode
    ...
    def metric_fieldnames(self):
        return [
            ...
            'sdf_residual_mean', 'sdf_residual_abs_mean', 'sdf_residual_abs_max'
        ]
    ...
    def collect_train_metrics(self, frame_idx, loss, image_loss, eikonal_loss,
        ...
        sdf_residual_mean = render_out.get('sdf_residual_mean')
        sdf_residual_abs_mean = render_out.get('sdf_residual_abs_mean')
        sdf_residual_abs_max = render_out.get('sdf_residual_abs_max')
        return {
            'iter': self.iter_step,
            ...
            'sdf_residual_mean': self.tensor_stat(sdf_residual_mean, torch.mean),
            'sdf_residual_abs_mean': self.tensor_stat(sdf_residual_abs_mean, torch.mean),
            'sdf_residual_abs_max': self.tensor_stat(sdf_residual_abs_max, torch.max),
        }
    ...
    def validate_mesh(self, resolution=256, threshold=0.0):
        if self.model_type == 'nerf':
            if getattr(self.nerf_network, 'use_ellipsoid_surface', False):
                threshold = 0.0
            elif threshold == 0.0:
        ...
        elif getattr(self.nerf_network, 'use_ellipsoid_surface', False):
            vertices, triangles = self.renderer.extract_nerf_surface_geometry(
